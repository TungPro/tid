...
abstract class OAuth2 {
	...
	abstract protected function setAccessToken($oauth_token, $client_id, $expires, $scope = NULL, $user_id = NULL);
	...
	public function grantAccessToken() {
		...
		$token = $this->createAccessToken($client[0], $input["scope"], $input['user_id']);
	...
	protected function createAccessToken($client_id, $scope = NULL, $user_id = NULL) {
		$token = array(
		...
		$this->setAccessToken($token["access_token"], $client_id, time() + $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME), $scope, $user_id);
